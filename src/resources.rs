...
// converts an Io error into our error
...
impl Resources {
    pub fn from_relative_exe_path(rel_path: &Path) -> Result<Resources, Error> {
        ...
        let exe_path = exe_file_name.parent() // get the parent directory of the exe path
    ...
    pub fn load_cstring(&self, resource_name: &str) -> Result<ffi::CString, Error> {
        ...
        // allocate buffer of same size as file
...
fn resource_name_to_path(root_dir: &Path, location: &str) -> PathBuf {
    let mut path: PathBuf = root_dir.into(); // into() calls the from function of Path to PathBuf b/c path was specifically typed as a PathBuf.
